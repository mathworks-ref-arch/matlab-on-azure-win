...
## **Introduction**
This guide shows how to build your own Azure&reg; Virtual Hard Disk (VHD) using the same scripts that form the basis of the build process for MathWorks&reg; prebuilt images.
...
You also need to specify a resource group and storage account to store the custom artifact under.
...
Packer writes its output, including the URL of the generated machine image, to a `manifest.json` file.
To use this URL to deploy the built image, see [Deploy Machine Image](#deploy-machine-image).
...
The `artifact_id` section shows the URL of the custom VHD generated by each Packer build.
